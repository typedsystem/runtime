...
from runtime.helper.mod import _is_local, _is_global
...
class mod:
    ...
    def locals(module: Entry) -> Dict:
        try:
            return {name: mod.get(name, module) for name in dir(mod.get(module)) if _is_local(name)}
        except Exception as e:
            raise ModErr(e)
    ...
    def globals(module: Entry) -> Dict:
        try:
            return {name: mod.get(name, module) for name in dir(mod.get(module)) if _is_global(name)}
        except Exception as e:
            raise ModErr(e)
